...
class CustomPostgresHook(BaseHook):
    ...
    def __init__(self, postgres_conn_id, **kwargs):
        self.postgress_conn_id = postgres_conn_id

    def get_conn(self):
        airflow_conn = BaseHook.get_connection(self.postgress_conn_id)
        ...
        self.postgress_conn = psycopg2.connect(host=self.host, user=self.user, password=self.password,
                                               dbname=self.dbname, port=self.port)
        return self.postgress_conn
